    ...
    async login (req, res) {
        try {
            ...
            if(validation.length > 0) {
                return res.status(HTTP_STATUS.OK).send(failure("Provide information correctly", validation));
            }
            ...
            if(!existingUser.verified) {
                return res.status(HTTP_STATUS.OK).send(failure("Verify you email and then log in"));
            }
            ...
            if(checkPassword) {
                ...
                return res.status(HTTP_STATUS.OK).send(success("Successfully logged in", userInfo));
            } else{
